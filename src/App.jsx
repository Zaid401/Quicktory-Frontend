...
function App() {
  ...
  // ✅ 1. month/day ko state me rakhenge taaki useEffect me use ho sake
  const [monthDay, setMonthDay] = useState({ month: null, day: null });

  const handleSearch = async () => {
    if (!date) return alert('Please select a date!');

    // 🟢 2. Date ko split karke month & day nikalna
    ...
    // Optional: agar aap apne backend (quicktory-backend) se data lena chahte ho
    try {
      setLoading(true);
  ...
  useEffect(() => {
    // month/day tabhi fetch kare jab user ne date select kiya ho
    ...
    const fetchEvents = async () => {
      try {
        ...
        // Wikipedia se images lene ka step
            ...
            {events.map((ev, idx) => (
              ...
              >
                <EventCard
                  ...
                  links={ev.wikipedia}
                  image={ev.image} // 👈 yahan thumbnail milega
                />
              </motion.div>
            ))}
          </div>
...
//       setEvents(eventsWithImages); // yahan events me image field bhi aayegi
//     };
...
//           className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold text-gray-900 dark:text-gray-100"
//         >
//           📅 Pick a date, unlock history — <span className='text-amber-600'>instantly!</span>
//         </motion.h1>
//         <motion.div
//           initial={{ opacity: 0, y: 20 }}
//           animate={{ opacity: 1, y: 0 }}
...
//               >
//                 <EventCard
//                   key={ev.wikipedia[0]?.title || ev.description}
//                   year={ev.year}
//                   title={ev.wikipedia[0]?.title}
//                   description={ev.description}
//                   links={ev.wikipedia}
//                   image={ev.image}       // 👈  yeh naya prop
//                 />
//               </motion.div>
//             ))}
//           </div>
//         ) : (
//           <p className="text-center italic text-gray-500 dark:text-gray-400 mt-12">Please pick a date!</p>
//         )}
//       </section>
